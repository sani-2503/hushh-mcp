    ...
    case 'tools/call': {
      ...
      if (params.name === 'get_receipt_data') {
        try {
          ...
          const url = `${HUSHH_BASE}/api/v1/get-receipt-data?phone_number=${encodeURIComponent(phone_number)}`;
      ...
      if (params.name === 'get_browsing_data') {
        try {
          ...
          const url = `${HUSHH_BASE}/api/v1/browsing-data?phone_number=${encodeURIComponent(phone_number)}`;
          const browseRes = await fetch(url, {
            method: 'POST',
            headers: {
              accept: 'application/json',
      ...
      if (params.name === 'get_food_data') {
        try {
          ...
          const url = `${HUSHH_BASE}/api/v1/food-data?phone_number=${encodeURIComponent(phone_number)}`;
      ...
      if (params.name === 'get_insurance_data') {
        try {
          ...
          const url = `${HUSHH_BASE}/api/v1/insurance-data?phone_number=${encodeURIComponent(phone_number)}`;
      ...
      if (params.name === 'get_brand_preference') {
        try {
          ...
          const url = `${HUSHH_BASE}/api/v1/brand-preferences?phone_number=${encodeURIComponent(phone_number)}&brand_name=${encodeURIComponent(brand_name)}`;
          const prefRes = await fetch(url, {
            method: 'POST',
            headers: {
              accept: 'application/json',
      ...
      if (params.name === 'insert_fashion_data') {
        try {
          ...
          const url = `${HUSHH_BASE}/api/v1/insert-fashion-data?phone_number=${encodeURIComponent(phone_number)}`;
          const body = JSON.stringify({ answers });
