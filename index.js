    ...
    case 'tools/call': {
      ...
      if (params.name === 'get_consented_cards') {
        try {
          ...
          const url = `https://hushh-api-53407187172.us-central1.run.app/api/v1/list-consented-cards?phone_number=${encodeURIComponent(phone_number)}`;
          const cardsRes = await fetch(url, {
            method: 'POST',
            headers: {
              accept: 'application/json',
      ...
      if (params.name === 'insert_receipt_data') {
        try {
          ...
          const url = `https://hushh-api-53407187172.us-central1.run.app/api/v1/insert-receipt-data?phone_number=${encodeURIComponent(phone_number)}`;
